...
count_data <- read_tsv('F:/Bversity/Mini Project/GSE194150_raw_counts_GRCh38.p13_NCBI.tsv/GSE194150_raw_counts_GRCh38.p13_NCBI.tsv')
...
Type <- c("control_1", "control_1", "treatment_1", "control_3", "control_4", "treatment_3", "treatment_4")  # Define conditions
...
write.csv(res_df, "F:/Bversity/Mini Project/GSE194150_raw_counts_GRCh38.p13_NCBI.tsv/DESeq2_results.csv", row.names = TRUE)
...
gene_results <- data.frame(
  Gene = paste0("Gene", 1:1000), # Simulated gene names
  logFC = rnorm(1000, mean = 0, sd = 2),  # Simulated log Fold Change
  adj.P.Val = runif(1000, min = 0, max = 0.1) # Adjusted P-values (FDR)
)
significant_genes <- gene_results[gene_results$adj.P.Val < 0.05, ]
library(ggplot2)
ggplot(significant_genes, aes(x = logFC)) +
  geom_histogram(bins = 30, fill = "blue", alpha = 0.7, color = "black") +
  labs(title = "Histogram of Significant Genes (logFC)",
       x = "Log Fold Change (logFC)",
       y = "p - adjusted value (padj)") +
  theme_minimal()
# Plot heatmap
library(pheatmap)
...
sample_annotations <- data.frame(
  Treatment = c(rep("control", 4), rep("treatment", 3))
)
...
custom_colors <- c("control" = "skyblue", "treatment" = "pink")
...
head(res_df)
write.csv(res_df, "F:/Bversity/Mini Project/GSE194150_raw_counts_GRCh38.p13_NCBI.tsv/DESeq2_results_with_annotations.csv", row.names = TRUE)
